...
- `claude/` — Claude Code user config, symlinked into `~/.claude/` (top-level files individually, `skills/` and `agents/` each as a whole directory). Config here may depend on CLI tools (e.g. `jq` for the statusline, `rtk` for the Bash hook) — add those to `Brewfile` so a fresh `make` produces a working setup. Never symlink or track the rest of `~/.claude/` — it's machine state (session transcripts, history, caches) and must stay local. Some skills are vendored third-party snapshots; the rest are personal. The exception is `skills/synced/` — Claude Code's own plugin-sync state, which lands inside the repo only because `skills/` is symlinked wholesale; it's generated machine state, gitignored for that reason, and the ignore rule must stay. `agents/` holds custom subagent types — notably `code-reviewer`, a read-only reviewer that is the **single generator of review findings**. All three review skills delegate finding-generation to it and differ only in disposition: `/review` presents the findings and interactively fixes the ones the user picks; `/polish` fixes every finding and loops until M consecutive clean reviews or N runs (defaults: N=10, M=2; both overridable as `/polish N M`); `/ship` gates on a review with no blocking findings (any Issue or `(defect)` Nitpick blocks; `(preference)` nits ride along as advisory), then commits and advances the task (the automated Finish gate — the counterpart to `/forge`, which composes `/next` then `/polish` and stops at In Review). Shared source-of-truth files under `skills/`: `review-core.md` (the finding threshold/bar), `review-diff.sh` (diff base selection), and `verify-core.md` (the green-baseline check set, used by `/polish` and `/ship`).
