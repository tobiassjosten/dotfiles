...
- `claude/notify-stop.sh` is the `Stop` hook: it rings the tmux pane's bell when Claude finishes, but only when the main session is genuinely idle. Claude Code parks the turn and stops it whenever work is still in flight — a backgrounded subagent, a workflow, a `run_in_background` shell task — so an unconditional bell fires several times per piece of work. The script reads the hook payload and stays quiet while `background_tasks` holds anything bounded (subagent, workflow, shell, teammate, MCP task, cloud session), or while `session_crons` holds a one-shot wakeup that will resume the session on its own. Ambient watchers (`monitor`, `dream`) and recurring crons don't count as unfinished work, since either could run for the rest of the session and mute it — the exclusion list is `AMBIENT_TYPES` at the top of the script. A missing `jq` or an unreadable payload falls back to ringing, so the notification never goes silent by accident. The separate `Notification` hook keeps its own unconditional bell for permission prompts.
